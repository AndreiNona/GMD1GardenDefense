...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        horizontal = _playerInput.actions["Move"].ReadValue<Vector2>().x;
        vertical = _playerInput.actions["Move"].ReadValue<Vector2>().y;
    ...
    {
        bool buttonA = _playerInput.actions["A"].WasPressedThisFrame();
        bool buttonB = _playerInput.actions["B"].WasPressedThisFrame();
        bool buttonX = _playerInput.actions["X"].WasPressedThisFrame();
        bool buttonY = _playerInput.actions["Y"].WasPressedThisFrame();
        
        if(buttonA && !IsChasableInTrigger())
            SpawnObject();
        if(buttonB)
            Launch();
        if(buttonX)
